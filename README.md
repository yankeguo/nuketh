...
Export table to CSV files to Google Storage and download them, remember to add a asterisk to export desination filename since it exceeds the maximum allowd single file size (1G).

### 2. Build a Binary Addresses File

Since Ethereum address is 20-bytes width, we use 3 Uint64 number (24-bytes width) for better comparasion.

This will be our Bianry Addresses File (BAF).

#### 2.1 Generate a Unsorted Binary Addresses File

TODO: Orignal works in Golang

#### 2.2 Generate a Sorted Bianry Addresses File
